    ...
    for j in tuple[0]:
        for k in tuple[1]:
            if minimum >= (k - j) > 0 and k > j:
                ...
                minimum = k - j
    #adding 1 for right parenthesis to be included
...
def compute_par(string):
    ...
    try:
        float(equation)
        print(equation, " returned.")
        return equation
    except ValueError:
        #print(equation, " moved to counting function")
...
def compute_trigon(initial_string):
    ...
    #print("result", initial_string)
    return del_letters(initial_string)
    ...
    while '!' in initial_string:
        ...
        while ch.isdigit() or ch == '.':
            ...
            if left_side_string_index >= 0:
                ...
                left_side_string_index += 1
                ch = 'anything'
        maybe_integer = maybe_integer[::-1]
        #print("An integer=", maybe_integer)
        left_string = initial_string[0:left_side_string_index + 1]
        right_string = initial_string[right_side_string_index:len(initial_string)]
        #print("left_string", left_string)
        #print("integer=", maybe_integer)
...
def compute_two_operands_operations(initial_string, operations_list):
    ...
    for i, operator in enumerate(operators):
        ...
        if operator == '^' and left_side_string_index != 0:
            left_side_string_index += 1
        #print("left_side_string_index", left_side_string_index)
        ...
        #print("while ", operator_to_text(operator))
        #print("left_string", left_string)
...
def get_right_or_left_operand(initial_string, index, operand_type):
    ...
    if operand_type == Operand.LEFT:
        ...
        #print("maybe_number", maybe_number)
        maybe_number = maybe_number[::-1]  #reverse
        #print("maybe_number_rev", maybe_number)
